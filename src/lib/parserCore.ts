// Pure parsing routines shared by the parser Web Worker (normal path) and the
// main-thread fallback in parser.ts (used only when Workers are unavailable).

export type ProgressFn = (p: number) => void;

// Above this size, skip JSON.parse + pretty-print: the parsed object graph and
// re-serialized string can multiply memory usage several times over and OOM the tab.
const JSON_PRETTY_PRINT_LIMIT = 16 * 1024 * 1024;

export async function parseFileCore(file: File, onProgress?: ProgressFn): Promise<string> {
  const ext = file.name.split('.').pop()?.toLowerCase();

  if (ext === 'json') {
    return parseJson(file, onProgress);
  }
  if (ext === 'bin' || ext === 'dat') {
...
async function parseJson(file: File, onProgress?: ProgressFn): Promise<string> {
  const text = await parseTxt(file, onProgress);
  if (file.size > JSON_PRETTY_PRINT_LIMIT) {
    return text;
  }
  try {
...
async function parseBin(file: File, onProgress?: ProgressFn): Promise<string> {
...
async function parseMht(file: File, onProgress?: ProgressFn): Promise<string> {
  const chunkSize = 1024 * 1024; // 1MB chunks
  const totalSize = file.size;
  let offset = 0;
  const chunks: string[] = [];
...
async function parseTxt(file: File, onProgress?: ProgressFn): Promise<string> {
  const chunkSize = 1024 * 1024; // 1MB chunks
  const totalSize = file.size;
  let offset = 0;
  const chunks: string[] = [];
